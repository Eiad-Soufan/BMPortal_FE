...
function TrophyIcon() {
    return (
        <svg width="20" height="20" viewBox="0 0 24 24" aria-hidden="true">
            <path
                d="M7 4h10v3a5 5 0 0 1-4 4.9V14h2v2H9v-2h2v-2.1A5 5 0 0 1 7 7V4Z"
                fill="#9fa8da"
            />
            <path
                d="M5 5h2v3a3 3 0 0 1-3 3H3V7a2 2 0 0 1 2-2Zm12 0h2a2 2 0 0 1 2 2v4h-1a3 3 0 0 1-3-3V5Z"
                fill="#c5cae9"
            />
        </svg>
    );
            ...
            {featured ? (
                <div className="hb-main">
                    <div className="hb-avatar">
                        {featured.image ? (
                            <img src={absMedia(featured.image)} alt={featured.name} />
                        ) : (
                            <span>{(featured.name || '?').charAt(0)}</span>
                        )}
                    </div>
                    <div className="hb-name">{featured.name}</div>

                    {others.length > 0 && (
                        <div className="hb-others" aria-label="مرشحون آخرون">
                            {others.map((p, idx) => (
                                <div key={idx} className="hb-chip" title={p.name}>
                                    {p.image ? (
                                        <img src={absMedia(p.image)} alt={p.name} />
                                    ) : (
...
export default function HonorBoard({ mount = 'header' }) {
    ...
    const fetchData = async (forceOpen = false) => {
        ...
        try {
            ...
            if (attemptsRef.current < 3) {
                setTimeout(() => fetchData(forceOpen), 1500);
            }
        } finally {
    ...
    // تحميل البيانات عند الدخول أو تغيير المستخدم
    useEffect(() => {
        if (!token) return;
        if (prevKeyRef.current && prevKeyRef.current !== closeKey) {
            // مستخدم مختلف -> افتح اللوحة من جديد
            sessionStorage.removeItem(prevKeyRef.current);
            setOpen(true);
        }
        prevKeyRef.current = closeKey;
        fetchData(true);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [token, closeKey]);

    // إعادة الفتح عند العودة للنافذة أو استقبال event خارجي
    useEffect(() => {
        const onFocus = () => {
    ...
    return (
        ...
        .hb-card{
          border-radius:12px; border:1.5px solid rgba(230,195,108,.7);
        ...
        .hb-empty{
          font-size:12px; color:#4b5563; text-align:center; padding:4px 0 2px;
        }

        .hb-avatar{
          width:58px; height:58px; border-radius:999px;
          display:grid; place-items:center; background:#fef6dd;
        ...
        .hb-chip{
          ...
          display:grid; place-items:center;
          font-size:10px; background:#fffef6;
          box-shadow:0 1px 3px rgba(0,0,0,.12); border:1px solid #e6c36c;
        }
